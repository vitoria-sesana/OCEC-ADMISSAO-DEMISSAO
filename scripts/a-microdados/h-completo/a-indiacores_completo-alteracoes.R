...
tab <- left_join(tab, qnt_2006_2023, by = c("setor_criativo"))
tab <- copy(tab)
vetor <- lag(tab$mes)
tab <- setorder(tab,"setor_criativo", -"ano", -"mes")
tab[, total_emprego_final_mes := qntd_total_clt_dez_ano_anterior - cumsum(var_abs), 
    by = c("setor_criativo")]
tab[, total_emprego_comeco_mes := lag(total_emprego_final_mes)]
# tab[, total_emprego_comeco_mes := fcase(
#   is.na(total_emprego_comeco_mes) & setor_criativo == "Setor criativo", qnt_2006_2023$qntd_total_clt_dez_ano_anterior[1],
#   is.na(total_emprego_comeco_mes) & setor_criativo == "Setor não criativo", qnt_2006_2023$qntd_total_clt_dez_ano_anterior[2],
#  !is.na(total_emprego_comeco_mes), total_emprego_comeco_mes
# )]

tab[setor_criativo=="Setor criativo" & ano==2024 & mes==12,total_emprego_comeco_mes:=qnt_2006_2023$qntd_total_clt_dez_ano_anterior[1]]
tab[setor_criativo=="Setor não criativo" & ano==2024 & mes==12,total_emprego_comeco_mes:=qnt_2006_2023$qntd_total_clt_dez_ano_anterior[2]]
...
tab[, taxa_rotatividade := (min_adm_dem/total_emprego_final_mes)*100]
tab
